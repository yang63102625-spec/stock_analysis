...
# ---------------------------------------------------------------------------
# Cache backed by the unified TTLCache (was a custom FundamentalsCache dataclass).
# Each entry has its own TTL; ``None`` results are returned but not cached so
# callers transparently retry on the next call.
# ---------------------------------------------------------------------------

_cache = TTLCache(name="fundamentals")
_TTL_PLEDGE = 7 * 86400
...
# Cache keys
_KEY_PLEDGE = "pledge"
_KEY_GOODWILL = "goodwill"
_KEY_FORECAST = "forecast_negative"


def _load_pledge(api: Any) -> Dict[str, float]:
    try:
...
def _ensure_pledge(api: Any) -> Dict[str, float]:
    ...
    cached = _cache.get(_KEY_PLEDGE)
    if cached is not None:
        return cached
    mapping = _load_pledge(api)
    if mapping:
        _cache.set(_KEY_PLEDGE, mapping, _TTL_PLEDGE)
    return mapping


def _load_goodwill(api: Any) -> Dict[str, float]:
    try:
...
def _ensure_goodwill(api: Any) -> Dict[str, float]:
    ...
    cached = _cache.get(_KEY_GOODWILL)
    if cached is not None:
        return cached
    mapping = _load_goodwill(api)
    if mapping:
        _cache.set(_KEY_GOODWILL, mapping, _TTL_GOODWILL)
    return mapping


def _load_recent_reductions(api: Any, days: int) -> Set[str]:
    try:
...
def _ensure_recent_reductions(api: Any, days: int = VETO_REDUCTION_DAYS) -> Set[str]:
    ...
    # Different ``days`` produce different result sets; key on the parameter.
    key = ("recent_reductions", days)
    cached = _cache.get(key)
    if cached is not None:
        return cached
    codes = _load_recent_reductions(api, days)
    if codes:
        _cache.set(key, codes, _TTL_REDUCTION)
    return codes


def _load_negative_forecasts(api: Any) -> Set[str]:
    try:
        from datetime import datetime, timedelta
        end = datetime.now().strftime("%Y%m%d")
...
def _ensure_negative_forecasts(api: Any) -> Set[str]:
    ...
    cached = _cache.get(_KEY_FORECAST)
    if cached is not None:
        return cached
    codes = _load_negative_forecasts(api)
    if codes:
        _cache.set(_KEY_FORECAST, codes, _TTL_FORECAST)
    return codes
