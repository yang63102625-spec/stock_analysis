...
- **代码硬规则**（行宽 / 注释 / 模块拆分 / SSOT / 异常处理 / 线程安全等）见 [.cursor/rules/code-quality.mdc](.cursor/rules/code-quality.mdc) 与 [.cursor/rules/code-generation.mdc](.cursor/rules/code-generation.mdc)，本文件不再重复。
- Git 约束：
