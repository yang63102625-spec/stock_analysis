...
- 单一版本原则（Single Source of Truth）：
  - 仓库只保留每个模块的最新实现。重命名 / 移动 / 拆分模块时，**禁止**留下兼容 shim、re-export 占位文件或"旧路径转发"模块。
  - 重构步骤必须在同一次提交中完成：① 把实现迁移到新规范路径；② 用 `rg` 搜出所有旧路径调用方（生产代码、测试、脚本、CI、文档）并改为新路径；③ `git rm` 删除旧文件；④ 跑测试确认无引用残留。
  - 例外：对外契约（REST API、CLI 参数、环境变量、数据库 schema）按常规弃用流程处理，不受此规则约束。
- 风格约束：
  - 行宽 120
  - `black` + `isort` + `flake8`
